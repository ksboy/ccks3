...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            batch_labels_cloned = batch_labels.clone()
            batch_labels[batch_labels==pad_token_label_id] = label_map['O']
            inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": batch_labels}
            if args.model_type != "distilbert":
                inputs["token_type_ids"] = (
                    batch[2] if args.model_type in ["bert", "xlnet"] else None
                )  # XLM and RoBERTa don"t use token_type_ids
            outputs = model(**inputs)
            tmp_eval_loss, logits, tmp_preds = outputs[:3]
        ...
        nb_eval_steps += 1
        if preds is None:
            preds = np.array(tmp_preds)
            out_label_ids = batch_labels_cloned.detach().cpu().numpy()
        else:
            preds = np.append(preds, tmp_preds, axis=0)
            out_label_ids = np.append(out_label_ids, batch_labels_cloned.detach().cpu().numpy(), axis=0)
...
def main():
    ...
    unexpected_keys = ['classifier.weight', 'classifier.bias', 'crf.start_transitions', 'crf.end_transitions', 'crf.transitions']
    ...
    # Training
    if args.do_train:
        train_dataset = load_and_cache_examples(args, tokenizer, labels, label_map['O'], mode="train")
